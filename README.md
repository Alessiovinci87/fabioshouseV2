...
- **Spagnolo** (agosto 2026): quinta lingua su `/es/…`. Le traduzioni NON stanno in `i18n.js`/`data.js` ma in `tools/lang/es/*.json` (chiave → testo: `dict:<chiave>` per il dizionario, `data.<percorso>` per FH_DATA); `node tools/merge-lang.js es tools/lang/es/*.json` genera `lang-es.js`, caricato dopo `data.js`, che aggiunge `es` a runtime. Per trovare le voci ancora da tradurre (es. dopo aver aggiunto un luogo): `node tools/extract-i18n.js es` stampa le stringhe IT senza spagnolo. Lo stesso meccanismo vale per una eventuale sesta lingua.
- `hreflang` IT/EN/FR/DE/ES + `x-default`, per rotta
